...
2. 왼쪽 사이드바 하단의 본인 이름을 클릭한 뒤, **"Customize"**를 선택합니다.
3. **"Connectors"** 탭을 클릭합니다.
4. 오른쪽의 **"+"** 버튼을 누르고 **"Add custom connector"**를 선택합니다.
5. URL 입력란에 아래 주소를 붙여넣습니다. `honggildong` 부분을 **0단계에서 발급받은 본인 인증키**로 바꾸세요:
...
7. 채팅 화면으로 돌아갑니다.
8. 메시지 입력창 왼쪽의 **"+"** 버튼을 클릭합니다.
9. **"Connectors"**를 선택하고, **korean-law**를 켜짐(토글 ON) 상태로 만듭니다.
10. 이제 "근로기준법 제74조 알려줘"라고 입력하면 AI가 법령을 검색해줍니다!

> **참고**: 한번 등록하면 다음부터는 7~9번만 하면 됩니다. 커넥터를 수정하려면 삭제 후 다시 추가해야 합니다.
